
// Generated by 'transfrom'
/* eslint-disable */
import React from 'react';
import { Svg } from 'react-sketchapp';
const { Path, default: SvgContainer } = Svg;
export default (props) => (
  <SvgContainer class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" name="fastforward" width={ props.width || 16 } height={ props.height || 16 }
    fill={ props.fill || "#666" }>
    <Path d="M499.4 481.6 63.1 106.1C38.7 83.4 0 101.5 0 135.6l0 752.7c0 34.2 38.7 52.3 63.1 29.5l436.2-377.3c8.4-7.9 12.6-18.7 12.7-29.5C512 500.2 507.8 489.4 499.4 481.6zM988 96 988 96c-19.9 0-36 16.1-36 36l0 298.5L575.1 106.1c-24.4-22.8-63.1-4.7-63.1 29.5l0 375.4 0 377.3c0 34.2 38.7 52.3 63.1 29.5l376.9-326L952 892c0 19.9 16.1 36 36 36l0 0c19.9 0 36-16.1 36-36L1024 132C1024 112.1 1007.9 96 988 96z"
      p-id="4980" />
  </SvgContainer>
)
