
// Generated by 'transfrom'
/* eslint-disable */
import React from 'react';
import { Svg } from 'react-sketchapp';
const { Path, default: SvgContainer } = Svg;
export default (props) => (
  <SvgContainer class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" name="arrowsalt" width={ props.width || 16 } height={ props.height || 16 }
    fill={ props.fill || "#666" }>
    <Path d="M429.9 872.8 218.7 872.8l232.1-232.1c17.2-17.2 17.2-45 0-62.2-17.2-17.2-45-17.2-62.2 0L152.8 814.2 152.8 604.8c0-24.3-19.7-44-44-44-24.3 0-44 19.7-44 44l0 312c0 24.3 19.7 44 44 44l321 0c24.3 0 44-19.7 44-44S454.2 872.8 429.9 872.8zM915.9 64.8l-321 0c-24.3 0-44 19.7-44 44 0 24.3 19.7 44 44 44L806 152.8 573.9 384.8c-17.2 17.2-17.2 45 0 62.2 8.6 8.6 19.9 12.9 31.1 12.9 11.3 0 22.5-4.3 31.1-12.9l235.7-235.7 0 209.4c0 24.3 19.7 44 44 44s44-19.7 44-44l0-312C959.9 84.5 940.2 64.8 915.9 64.8z"
      p-id="8692" />
  </SvgContainer>
)
